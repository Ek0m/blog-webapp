...
const Post = ({post}: Props) => {
  ...
  return (
    <div>
        <Header />
            <img className=' w-full h-96 object-cover' src={urlFor(post.mainImage).url()!} alt=""  />
            <div className=' max-w-3xl mx-auto mb-10'>
                <article className=' w-full mx-auto p-5 bg-secondaryColor/10' >
                    ...
                    <div className=' flex items-center gap-2'>
                        <img className=' rounded-full w-12 h-12 object-cover bg-red-400' src={urlFor(post.author.image).url()!} alt="" />
                    ...
                    <div className='mt-10'>
                        <PortableText 
                            ...
                            projectId={process.env.NEXT_PUBLIC_SANITY_PROJECT_ID || "pmtklekf"}
                            content={post.body}
                            serializers={{
                                h1: (props: any) => (
                                    <h1 
                                ...
                                link: ({href, children}: any) => (
                                    <a href={href} className=' text-cyan-500 hover:undeline'>
                                        {children}
                                    </a>
                                ),
                ...
                
                {
                    submitted ? <div className=' flex flex-col items-center gap-2 p-10 my-10 bg-bgColor text-white mx-auto'>
                        ...
                        <p>Once its been approved, it'll appear</p>
                    </div> : <div>
                    <p className='text-xs text-secondaryColor uppercase font-titleFont font-bold'>Enjoyed this post?</p>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className='mt-7 flex flex-col gap-6' >
                        <label className='flex flex-col'>
                            ...
                            <input {...register('name', { required: true })} className='text-base placeholder:text-sm border-b-[1px] border-secondaryColor py-1 px-4 outline-none focus-within:shadow-xl shadow-secondaryColor' type='text' placeholder='Enter Your Name' />
                        </label>

                        <label className='flex flex-col'>
                            <span className='font-titleFont font-semibold text-base'>Email</span>
                            <input {...register('email', { required: true })} className='text-base placeholder:text-sm border-b-[1px] border-secondaryColor py-1 px-4 outline-none focus-within:shadow-xl shadow-secondaryColor' type='email' placeholder='Enter Your email' />
                        </label>

                        <label className='flex flex-col'>
                            <span className='font-titleFont font-semibold text-base'>Comment</span>
                            <textarea {...register('comment', { required: true })} className='text-base placeholder:text-sm border-b-[1px] border-secondaryColor py-1 px-4 outline-none focus-within:shadow-xl shadow-secondaryColor'  placeholder='Enter Your comment' rows={6} />
                        </label>
